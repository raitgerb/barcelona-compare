---
name: "GOOD NAILS BY RAFFEL PAGÈS"
neighborhood: "Barcelona"
address: "Av. de la Verge de Montserrat, 84, 08820 El Prat de Llobregat, Barcelona, Spain"
phone: "604 97 44 56"
website: "https://goodnailsbyrp.com/"
priceIndicator: "€€"
hours:
  monday: "09:00-20:30"
  tuesday: "09:00-20:30"
  wednesday: "09:00-20:30"
  thursday: "09:00-20:30"
  friday: "09:00-20:30"
  saturday: "09:00-14:00"
languages:
  - "Español"
  - "Inglés"
googleRating: 4.6
googleReviewCount: 190
googlePlaceId: "ChIJPWEvzvyfpBIRNSUocTQsNSw"
googleReviews:
  - author: "Milene Correia"
    rating: 3
    relativeTime: "11 months ago"
    languageCode: "en"
    text: "Nice express shop on the Main Street, however the quality of the nails could be better. I paid for a full pedicure and she didn’t even dip my feet in water, only wrapped it in a towel and then began to remove the skin. But it wasn’t very productive nor enjoyable.  I also got French on my hands and the nail chipped the day after, the colour selected for the pink French actually doesn’t suit me. But overall nice place."
  - author: "Maria Ramirez"
    rating: 5
    relativeTime: "2 years ago"
    languageCode: "en"
    text: "I arrived with horrible nails. Amazing treatment, we choose a minty colour and Daian fixed one of my nails that was broken. Super nice and lovely staff. I will definitely be back !"
  - author: "Lorena Perez Sanchez"
    rating: 5
    relativeTime: "a year ago"
    languageCode: "en"
    text: "Maeavillosas"
  - author: "Eva María"
    rating: 5
    relativeTime: "2 months ago"
    languageCode: "es"
    text: "He ido hoy por primera vez y super bien,me han echo retirada de unas uñas de gel echas en otro centro y me han echo la semipermanente,el color recomendado por la chica (se me a olvidado preguntar el nombre) es precioso. El trato por todas es de 10 y el ambiente tambien,he estado muy agusto (cosa que ultimamente me costaba en otros centros). Sin duda,repetiré ☺️"
  - author: "Sara I"
    rating: 1
    relativeTime: "8 months ago"
    languageCode: "es"
    text: "No se por dónde empezar... Fatal todo, he salido muy descontenta. Me ha atendido Dolores, y aparte de que me ha puesto las pegatinas sin ton ni son (fijaros en los arbolitos, que no están centrados), donde me ha puesto la estrella fugaz se ve que no está bien adherida a la uña (se ve el papelito y arrugas, horrible!). Se lo he dicho y no me lo ha querido arreglar. Finalmente, la uña del dedo gordo, me ha puesto el diseño encima de la punta, teniendo espacio de sobra para colocar la pegatina bien (fijaros el trozo que sobra abajo). Aparte de esto, yo le he enseñado un diseño de lo que quería: uñas blancas opacas y el final con oro en topitos. Pues las uñas son semitransparentes, no blancas. Y, aunque le he insistido que quería topitos, en vez de darme una solución (porque tienen esmalte dorado con topitos) me ha insistido en ponerlas doradas lisas. Nada que ver con lo que buscaba. Aparte de esto, es muy antipática e insistente. No me escuchaba y, apesar de haber reservado 1h, no me dejaba hablar para expresarme e insistía en hacer lo que ella decía, todo con prisas. HORRIBLE. 25€ me ha cobrado por esto... dónde voy yo con la uña que se nota la pegatina mal puesta? En serio, no vayáis... tenéis otros lugares mucho mejores."
---
